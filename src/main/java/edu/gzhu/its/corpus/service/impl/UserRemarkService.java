...
public class UserRemarkService extends BaseDAOImpl<UserRemark, Long> implements IUserRemarkService {
	...
	public void saveUserRemark(UserRemark remark,long taskId) {
		...
		//查询本条用户评论是否已被其他用户标注
		List<UserRemark> remarks = this.find(" userComment_id="+remark.getUserComment().getId() +" and user_id <>" +remark.getId());
		if(remarks!=null){
			//当前只有一个人标注
			if(remarks.size()==1){
				...
				if(oldRemark.equals(remark)){
					this.executeSql("update user_comment set isAnnotationed=1 where id="+remark.getUserComment().getId());
				}else{
					//和其他人标注不同，交给第三人处理
					//找到标注任务最少的人
					List<Object[]> objects = this.findBySql("select t.uid,min(t.ucount) from (select user_id as uid,count(*) as ucount from user_task  group by user_id) t");
